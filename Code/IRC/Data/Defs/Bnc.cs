...
{
	public class Bnc : Platform.Data.Obj<Bnc>
	{
		...
		#region Properties
			public bool IsPredefined => bIsPredefined;
		...
		#region Methods
			protected void FirePropChanged(string strWhichProp) => PropertyChanged?.Invoke(this, new
				(strWhichProp));
			...
			{
				if(ussetPorts.Contains(usPortToRemove))
				{
					ussetPorts.Remove(usPortToRemove);
			...
			{
				if(!ussetSslPorts.Contains(usNewSslPort))
				{
					ussetSslPorts.Add(usNewSslPort);
			...
			{
				if(ussetSslPorts.Contains(usSslPortToRemove))
				{
					ussetSslPorts.Remove(usSslPortToRemove);
