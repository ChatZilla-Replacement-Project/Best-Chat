...
namespace BestChat.IRC.Data.Defs
{
	public class ServerInfo : Platform.Data.Obj<ServerInfo>
	{
		#region Constructors & Deconstructors
			...
			public ServerInfo(in Network netParent, in string strDomain, in System.Collections.Generic
				.IEnumerable<ushort> eusPorts, in System.Collections.Generic.IEnumerable<ushort> eusSslPorts)
			{
				this.netParent = netParent;
		...
		#region Members
			...
			private readonly System.Collections.Generic.SortedSet<ushort> setPorts = new();

			private readonly System.Collections.Generic.SortedSet<ushort> setSslPorts = new();
		#endregion

		#region Properties
			public string Domain => strDomain;
			...
			public bool IsEnabled
			{
				...
				protected set
				{
					if(netParent is not UserNetwork)
						throw new System.InvalidProgramException("ServerInfo instances owned by anything other than a " +
							"UserNetwork are readonly.");
			...
			public System.Collections.Generic.IReadOnlySet<ushort> Ports => setPorts;

			public string PortsAsText => string.Join(strPortDelimiter, setPorts);

			public System.Collections.Generic.IReadOnlySet<ushort> SslPorts => setSslPorts;

			public string SslPortsAsText => string.Join(strPortDelimiter, setSslPorts);
		#endregion
		...
		#region Methods
			...
			{
				if(!setPorts.Contains(usNewPort))
				{
					setPorts.Add(usNewPort);
			...
			{
				if(setPorts.Contains(usPortToRemove))
				{
					setPorts.Remove(usPortToRemove);
			...
			{
				if(!setSslPorts.Contains(usNewSslPort))
				{
					setSslPorts.Add(usNewSslPort);
			...
			{
				if(setSslPorts.Contains(usSslPortToRemove))
				{
					setSslPorts.Remove(usSslPortToRemove);
			...
			public Editable MakeEditableVersion(UserNetwork.Editable eunetParent) => new Editable(eunetParent,
				this);
		#endregion
