...
namespace BestChat.IRC.Data
{
	...
	public class Chan : AbstractConversation, Platform.TreeData.VisualTreeData.IItemInfo
	{
				...
				.strChanNameDescForTree.Fmt(strName, anetOwner.unetDef.Name))
			{
				...
				this.strName = strName;
					...
					System.Text.Json.JsonDocument doc = System.Text.Json.JsonDocument.Parse(client
						.GetStreamAsync(strName switch
						{
							"##space" => "https://github.com/ChatZilla-Replacement-Project/JSON-Data/blob/main/" +
								"Sample%20Data/Sample%20%23%23space%20log.json",
							"#best-chat" => "https://github.com/ChatZilla-Replacement-Project/JSON-Data/blob/main/" +
								"Sample%20Data/Sample%20%23best-chat%20log.json",
									...
									case "action":
										switch(elementCur.GetProperty("SubType").GetString())
										{
											...
											case nameof(Events.Types.ActionEventType.Types.topicChanged):
												RecordEvent(new Events.ActionEventInfo.TopicChangeActionEventInfo(ProperName,
													elementCur.GetProperty("NewTopic").GetString() ?? throw new System
													.Exception($"Topic change record from the sample log for {ProperName
													} is missing the required NewTopic field"), elementCur
													.GetProperty("SetBy").GetString() ?? throw new System
													.Exception($"Topic change record from the sample log for {ProperName
													} is missing the required SetBy field"), elementCur
													.GetProperty("SetAt").GetDateTime()));
											...
											case nameof(Events.Types.ActionEventType.Types.userOpped):
												RecordEvent(new Events.ActionEventInfo.UserOppedActionEventInfo(elementCur
													.GetProperty("WhoGotOp").GetString() ?? throw new System
													.Exception("user got op record from sample channel log is missing the " +
													"require WhoGotOp field"), elementCur.GetProperty("WhoIssuedOp")
													.GetString() ?? throw new System.Exception("Op issued record from sample" +
													" channel log data is missing the required WhoIssuedOp field")));
										...
										switch(elementCur.GetProperty("SubType").GetString())
										{
											case nameof(Events.Types.PostEventType.Types.say):
												RecordEvent(new Events.PostEventInfo.SayPostEventInfo(elementCur
													...
													.Exception("Record of /say in channel log is missing the required " +
													"NickOfSender field"), elementCur.GetProperty("Msg").GetString() ??
													throw new System.Exception("Required Msg field is missing from /say " +
													"record in sample channel log data")));
											...
											case nameof(Events.Types.PostEventType.Types.me):
												RecordEvent(new Events.PostEventInfo.MePostEventInfo(elementCur
													.GetProperty("NickOfSender").GetString() ?? throw new System
													.Exception("Record of /me in channel log is missing the required " +
													"NickOfSender field"), elementCur.GetProperty("Msg").GetString() ??
													throw new System.Exception("Required Msg field is missing from /me " +
													"record in sample channel log data")));
										...
										switch(elementCur.GetProperty("SubType").GetString())
										{
											case nameof(Events.Types.NoticeEventType.Types.info):
												RecordEvent(new Events.NoticeEventInfo
													.InfoNoticeEventInfo(elementCur.GetProperty(nameof(Events.ActionEventInfo
													.DescForEvt)).GetString() ?? throw new System.Exception("info event " +
													"from channel logged sample data is missing the required DescForEvent " +
													"field.")));
		...
		#region Delegates
			public delegate void DStrFieldChanged(in Chan chanSender, string strOldTopic, string strNewTopic);
		#endregion
		...
		#region Properties
			...
			public override bool IsTopicLocked => mapModesWithStdTypes.ContainsKey(Defs.ChanMode.StdModeTypes
				.TopicLock) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.TopicLock].StateAsBool;

			public override bool IsSecret => mapModesWithStdTypes.ContainsKey(Defs.ChanMode.StdModeTypes.Secret)
				&& mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.Secret].StateAsBool;

			public override bool IsPrivate => mapModesWithStdTypes.ContainsKey(Defs.ChanMode.StdModeTypes
				.Private) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.Private].StateAsBool;

			public override bool IsModerated => mapModesWithStdTypes.ContainsKey(Defs.ChanMode.StdModeTypes
				.Moderated) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.Moderated].StateAsBool;

			public override bool AreColorsStripped => mapModesWithStdTypes.ContainsKey(Defs.ChanMode
				.StdModeTypes.ColorStrip) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.ColorStrip]
				.StateAsBool;

			public override bool AreOutsideMsgProhibited => mapModesWithStdTypes.ContainsKey(Defs.ChanMode
				.StdModeTypes.NoOutsideMsg) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.NoOutsideMsg]
				.StateAsBool;

			public override bool IsInviteOnly => mapModesWithStdTypes.ContainsKey(Defs.ChanMode.StdModeTypes
				.InviteOnly) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.InviteOnly].StateAsBool;

			public override bool IsKeywordRequired => mapModesWithStdTypes.ContainsKey(Defs.ChanMode
				.StdModeTypes.Keyword) && mapModesWithStdTypes[Defs.ChanMode.StdModeTypes.Keyword].StateAsBool;
